...
jpgImagesDir = fullfile('Dataset', '*.jpg');
...
for i=1:total
   % disp(i)
    ...
    F = fullfile('Dataset',jpg_files(i).name);
   % s=strcat(num2str(i),'.png');
    %file=fullfile('Dataset',s);
    %file=fullfile(pathname,filename{i});
    gambar{i}=imread(F);
    if size(gambar{i}, 3) ~= 3
            gambar{i} = cat(3, gambar{i},gambar{i}, gambar{i});
        end
